...
the live order-book CLI, and the multi-market browser dashboard.
...
The dashboard reuses the same `OrderBook` and `KalshiWebSocket` logic as the
terminal watcher, but serves a local page so multiple markets can be watched
at once. If no tickers are provided, it automatically watches markets where
your portfolio currently has a non-zero position, then adds any tickers with
resting orders.

Each card now supports independent `YES`, `NO`, and `BOTH` modes, with `BOTH`
as the default. The browser UI also persists local controls for:
...
- `Edge Vs Mid` – whether last trade is stretched away from the current book midpoint
- `Depth 5¢` – near-touch liquidity on your side versus the opposite side
- `Imbalance` – top-of-book resting pressure

Use `--no-open` when you want the dashboard to stay in Positron Viewer instead
of auto-opening the external browser.
